...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!_controller.isGrounded)
        {
            _animator.SetFloat("jumpFallBlend", 3);
        }
        else
        {
            _animator.SetFloat("jumpFallBlend", 0);
        }
        ...
        {
            // Horizontal movement detected, play walking animation
            _animator.SetInteger("animState", 1);

        }
        else
        {
            // No horizontal movement detected, play idle animation
            _animator.SetInteger("animState", 0);
            // Assuming 0 is the index for idle animation
        }
